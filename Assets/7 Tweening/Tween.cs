...
public class Tween : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            case Graft.Linear:
                transform.position = Vector3.LerpUnclamped(initialPosition, targetPosition, relativeDistance);
                SpintRender.color = Color.LerpUnclamped(color1, color2, relativeDistance);
                break;
            case Graft.EaseInOutElastic:
                transform.position = Vector3.LerpUnclamped(initialPosition, targetPosition, EaseInOutElastic(relativeDistance));
                SpintRender.color = Color.LerpUnclamped(color1, color2, EaseInOutElastic(relativeDistance));
                break;
            case Graft.EaseInSine:
                transform.position = Vector3.LerpUnclamped(initialPosition, targetPosition, EaseInSine(relativeDistance));
                SpintRender.color = Color.LerpUnclamped(color1, color2, EaseInSine(relativeDistance));
                break;
            case Graft.easeOutBounce:
                transform.position = Vector3.LerpUnclamped(initialPosition, targetPosition, easeOutBounce(relativeDistance));
                SpintRender.color = Color.LerpUnclamped(color1, color2, easeOutBounce(relativeDistance));
                break;
            case Graft.curve:
                transform.position = Vector3.LerpUnclamped(initialPosition, targetPosition, curve.Evaluate(relativeDistance));
